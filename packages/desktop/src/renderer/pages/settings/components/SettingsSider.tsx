...
const SettingsSider: React.FC<{ collapsed?: boolean; tooltipEnabled?: boolean }> = ({
  ...
  return (
    <PerfProfiler id='settings.sider'>
      <div
            ...
            <React.Fragment key={item.id}>
              ...
              <Tooltip {...siderTooltipProps} content={item.label} position='right'>
                <div
                  ...
                  onClick={() => {
                    // Settings tab switch — the heavy work is the target page
                    // mount (data fetches, form re-init). The Profiler wraps the
                    // settings sider itself; the right-hand page mount is
                    // captured by `app.layout` (nested) once `<Outlet />` resolves.
                    mark('perf.settings.tab', 'tab_click', { id: item.id, path: `/settings/${item.path}` });
