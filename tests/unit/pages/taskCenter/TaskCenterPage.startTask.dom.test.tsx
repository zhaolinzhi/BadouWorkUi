...
vi.mock('@/renderer/pages/task-center/useTaskCenterT', () => ({
  useTaskCenterT: () => (key: string) => key,
}));

vi.mock('@/renderer/pages/settings/components/SettingsPageHeader', () => ({
  default: (_props: unknown) => <div data-testid='settings-page-header-stub' />,
}));
...
describe('TaskCenterPage — start task flow', () => {
  it('navigates to /guid with the 3-line prefill prompt when 开始任务 is clicked', async () => {
    render(
      <MemoryRouter>
        <TaskCenterPage />
      </MemoryRouter>
